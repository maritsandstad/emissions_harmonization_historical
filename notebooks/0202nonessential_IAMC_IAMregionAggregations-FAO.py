...
# %%
fao_history_file_national = DATA_ROOT / Path("national", "fao", "processed", f"fao_national_{FAO_PROCESSING_ID}.csv")
fao_history_file_global = DATA_ROOT / Path("national", "fao", "processed", f"fao_global_{FAO_PROCESSING_ID}.csv")

# file name for output
# TODO: add versioning / ID to this file
iamc_commondefinitions_regions_processed_output_file = DATA_ROOT / Path(
    "national", "fao", "processed", f"iamc_regions_fao_history_{IAMC_REGION_PROCESSING_ID}.csv"
)
iamc_commondefinitions_regions_history_missing_iso_vsFAO = DATA_ROOT / Path(
    "national", "fao", "processed", f"iamc_regions_fao_missing_iso_vsFAO_{IAMC_REGION_PROCESSING_ID}.csv"
)
iamc_commondefinitions_regions_history_missing_iso = DATA_ROOT / Path(
    "national", "fao", "processed", f"iamc_regions_fao_missing_iso_{IAMC_REGION_PROCESSING_ID}.csv"
)
...
# %%
fao_history[(fao_history["region"].isin(["usa"]))]
...
for _, row in region_mapping.iterrows():
    ...
    filtered_history = fao_history[(fao_history["region"].isin(iso_list))]
...
for r in [
    ...
    test_data = fao_history[(fao_history["region"].isin(test_countries))]
    ...
    temp_df = pd.DataFrame(
        {
            "model": m,
            "iso_list": [unique],
            "missing_vs_fao": [missing_vs_fao],
            "missing_from_fao": [missing_from_fao],
        }
    )
...
# %%
missing_iso.to_csv(iamc_commondefinitions_regions_history_missing_iso_vsFAO, index=False)
